 ...
 *
 * @category    Reea
 * @package     Reea_AdvancedSearch
 * @author      Magento Core Team <core@magentocommerce.com>
 * @modified    Alex Sava
 */

class Reea_Advancedsearch_Model_Resource_Advanced_Collection extends Mage_CatalogSearch_Model_Resource_Advanced_Collection
{
    /**
            ...
            foreach ($fields as $table => $conditions) {
                foreach ($conditions as $attributeId => $conditionValue) {
                    ...
                    if (!is_numeric($attributeId)) {
                        ...
                        if($attributeCode == "name" && array_key_exists("include",$conditionValue) && isset($conditionValue["include"])) {
                            $additionalAttributeId = (int)$conditionValue["include"];
                            $ids = array($attributeId ,$additionalAttributeId);
                            $select->where('t1.attribute_id IN (?)', $ids);
                        } else {
